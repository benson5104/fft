...
void readerThreadFunc(int numCol, MemoryMapped& dataChar) {
    ...
    while (true) {
        ...
        totalRead += batch_end - batch_start;
        
        if (logTime) {
            std::osyncstream(std::cout) << "Batch " << readThreadCount << " read : " << batch_end - batch_start << " ms"
                << " (start time : " << batch_start << " ms, end time : " << batch_end << " ms)\n";
        }
...
void processorThreadFunc(int numChannels) {
    ...
    while (true) {
        ...
        auto process_start = clock();
        FFTfunction(data, numChannels);
        auto process_end = clock();

        totalTime += process_end - process_start;
        if (logTime) {
            std::osyncstream(std::cout) << "Batch " << processorThreadCount << " process : " << process_end - process_start << " ms"
                << " (start time : " << process_start << " ms, end time : " << process_end << " ms)\n";
        }
...
int main() {
    ...
    std::cout << "Would you like to enable logging and create a log file (0: no, 1: yes): ";
    std::cin >> logTime;
